...
TEST_CASE("mesaac::measures::Tversky", "[mesaac]") {
  Tversky measure(0.5);

  SECTION("Get name") { REQUIRE(measure.name() == "Tversky"); }
  ...
  // TO CONSIDER  Test with abnormal alpha values?  Doing so won't
  // throw any exceptions, so maybe there's no point.
}
} // namespace mesaac::measures
