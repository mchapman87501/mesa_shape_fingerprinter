...
#include "Globals.h"  // For BitVector
#include <vector>
...
namespace mesaac {
    namespace shape {
        ...
        class VolBox;
        typedef boost::shared_ptr<VolBox> VolBoxPtr;
        ...
        public:
            ...
            void set_bits_for_spheres(
                const PointList& spheres, BitVector& bits, 
                bool from_scratch, unsigned int offset) const;
            
            void set_bits_for_one_sphere(
                const Point& sphere, BitVector& bits, 
                unsigned int offset) const;
            
            void get_points_within_spheres(
                const PointList& spheres, PointList& contained_points,
                unsigned int offset) const;
                
            // For folded fingerprints:
            void set_folded_bits_for_spheres(
                const PointList& spheres, BitVector& bits, 
                unsigned int num_folds, unsigned int offset) const;
        ...
        protected:
            ...
            void set_bits_for_one_sphere_unchecked(
                const Point& sphere, BitVector& bits, 
                unsigned int offset) const;
            void set_folded_bits_for_one_sphere_unchecked(
