...
public:
  ...
  void show_blurb() {
    cerr << "Running " << basename(m_argv[0]) << endl
         ...
         << "Version " << Version << " Creation Date: " << CreationDate << endl
         << "Expiration Date: " << mesaac::expirationDateStr() << endl;
  }

  void parse_args(ifstream &sdff, string &sdf_pathname, float &atom_scale,
                  float &max_radius) {
    int i = 1;
    while (i < m_argc) {
...
-h | --help       = Show this help message and exit"
  ...
  void error(ostringstream &outs) { error(outs.str()); }
