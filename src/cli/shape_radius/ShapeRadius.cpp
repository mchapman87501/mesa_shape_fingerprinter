...
static void showUsage(char *exename, string msg="") {
    cerr << "Usage: " << basename(exename)
         << " sd_file hamms_sphere_file atom_scale" << endl
         << "Find the maximum extent of any conformer in sd_file with respect to " << endl
         ...
         << "atom_scale        = the amount, in the range [1.0..2.0], by which to " << endl
...
void getMolCoords(mol::Mol& m, CoordsList& coords, float& x_sum, float& y_sum, float& z_sum)
{
    x_sum = 0.0;
    y_sum = 0.0; 
    z_sum = 0.0;
...
int main(int argc, char **argv){
    ...
    vector<float> x_extremes, y_extremes, z_extremes, atom_radius_extremes, radius_extremes;
    float max_boundary;
    ...
    while(reader.read(m)){
        ...
        //cout << "BEFORE PCA: " << max_x << " " << max_y << " " << max_z << " " << max_atom_radius << " " << max_radius << endl;
    ...
    //cerr << "Compoundcoords here " << CompoundCoords.size() << endl;
        ...
        for(i=0;i<HammsSphereSequenceSize; i++){
            for(k=0;k<CompoundCoords.size(); k++){
                ...
                if(fabs(HammsSphereCoords[i][0] - CompoundCoords[k][0]) <= max_boundary){
        ...
        for(i=0;i<CompoundCoords.size();i++){
            ...
            float curr_radius = sqrt((curr_x * curr_x) +
                                     (curr_y * curr_y) +
                                     (curr_z * curr_z));
            ...
            max_atom_radius = (max_atom_radius > curr_atom_radius) ? max_atom_radius : curr_atom_radius;
            max_radius = (max_radius > curr_radius) ? max_radius : curr_radius;
        }
        //cout << "AFTER PCA: " << max_x << " " << max_y << " " << max_z << " " << max_atom_radius << " " << max_radius << endl;
    ...
    float atom_radius_tmp = *max_element(atom_radius_extremes.begin(),atom_radius_extremes.end());
    cout << "Max_x " <<
        "Max_y " << 
        "Max_z " <<
        "Max_atom_radius " <<
    ...
    cout << *max_element(x_extremes.begin(),x_extremes.end()) << " " <<  
        ...
        *max_element(z_extremes.begin(),z_extremes.end()) << " " << 
        atom_radius_tmp << " " << 
        *max_element(radius_extremes.begin(),radius_extremes.end()) + atom_radius_tmp << endl;
