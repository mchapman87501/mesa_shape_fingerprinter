...
public:
    ...
    void get_args(string& sd_pathname, string& he_pathname, 
        string& hs_pathname, float& atom_scale, bool& include_ids, 
        SDFShapeFingerprinter::FormatEnum& format, unsigned int& num_folds,
...
protected:
    ...
    void get_value_for(string option_name, int& i, unsigned int& uival) {
        ...
        if (!(ins >> uival)) {
            ostringstream msg;
            msg << "Invalid integer value '" << sval  << "' for "
                << option_name;
            show_usage(msg);
        }
    }
    ...
    void show_usage(string err_msg = "", int status=1) {
        cerr << "Usage: " << basename(m_argv[0]) << " [options] "
             ...
             << "Generate shape + feature fingerprints for 3D conformers.\n\
\n\
...
int main(int argc, char **argv) {
    ...
    options.get_args(sd_pathname, hamms_ellipsoid_path, 
        hamms_spheroid_path, atom_scale, include_ids, format, num_folds,
