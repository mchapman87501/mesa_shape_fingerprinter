...
namespace mesaac {
    ...
    SDFShapeFingerprinter::SDFShapeFingerprinter(
        ...
        m_epsilonSqr(radiiEpsilon * radiiEpsilon),
    ...
    void SDFShapeFingerprinter::run(int startIndex, int endIndex) {
        ...
        readPoints(m_hammsEllipsoidPathname, "hamms_ellipsoid_filename",
            ellipsoid);
        readPoints(m_hammsSpherePathname, "hamms_sphere_filename",
            sphere);
    ...
    void SDFShapeFingerprinter::processMolecules(
        PointList& ellipsoid, PointList& sphere, int startIndex, int endIndex)
    {
        if (startIndex < 0)
        {
            cerr << "Invalid start index " << startIndex << " -- must be >= 0"
                << endl;
            exit(1);
        }
