...
namespace mesaac 
{
    ...
    class TestCase : public CppUnit::TestFixture
    {
        ...
        CPPUNIT_TEST(test_basic_folding);
        
        CPPUNIT_TEST_SUITE_END();
        
    protected:
        bool almost_equal(float expected, float actual, 
                          float max_fract=1.0e-6, bool verbose=false)
        {
                                  ...
                                  PointList& contained)
        {
            ...
            for (unsigned int i = 0; i != cloud.size(); i++) {
                ...
                for (unsigned int j = 0; j != centers.size(); j++) {
                    ...
                    float dx = p_cloud[0] - p_center[0],
                          dy = p_cloud[1] - p_center[1],
                          dz = p_cloud[2] - p_center[2],
                          r = p_center[3],
                          rsqr = r * r,
    ...
    public:
        ...
        void test_set_bits_for_one_sphere() {
            ...
            for (float x = -15.0; x != 15.0; x += 1.0) {
                BitVector vb_matches(sphere.size()), 
        ...
        void test_moving_spherule() {
            ...
            for (float center = -max_offset; center <= max_offset; center += d_center) {
                ...
                CPPUNIT_ASSERT(almost_equal(exp_cnt, contained.size(), 
                                            0.075));
            }
        }
        ...
        static void get_brute_force_folded(
            BitVector& src, BitVector& dest, unsigned int num_folds)
        {
        ...
        void test_basic_folding() {
            ...
            // unfolded.resize(vb.size());
            // unfolded.reset();
            // vb.set_folded_bits_for_spheres(center_spheres, unfolded, 0, 0);
            // 
            // CPPUNIT_ASSERT_EQUAL(full_fp, unfolded);
            // CPPUNIT_ASSERT(unfolded.count() > 0);
            // CPPUNIT_ASSERT(unfolded.count() <= sphere.size());
            
            const unsigned int max_folds = 5;
            unsigned int num_folds;
            unsigned int folded_size = vb.size();
            for (num_folds = 0, folded_size = vb.size(); 
                 num_folds != max_folds; ++num_folds, folded_size /= 2) {
                BitVector folded;
                folded.resize(folded_size);
                folded.reset();
                vb.set_folded_bits_for_spheres(
                    center_spheres, folded, num_folds, 0);
                CPPUNIT_ASSERT(folded.count() > 0);
                
                // cout << "Folds: " << num_folds << " = " << folded << endl;

                BitVector folded_brute;
                get_brute_force_folded(full_fp, folded_brute, num_folds);
                CPPUNIT_ASSERT_EQUAL(folded, folded_brute);
            }
        }
