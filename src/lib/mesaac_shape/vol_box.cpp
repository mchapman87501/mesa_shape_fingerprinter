...
namespace mesaac {
    namespace shape {
        ...
        void VolBox::add_points(const PointList& points) {
            ...
            for (i = points.begin(); i != points.end(); i++) {
                const Point& p(*i);
                int ix = (int)((p[0] - m_xmin) / m_dx),
                    iy = (int)((p[1] - m_ymin) / m_dy),
                ...
                xyz_bucket.at(ix).at(iy).at(iz).push_back(
                    m_bucket_points.size());
                m_bucket_points.push_back(p);
            }
        }
        
        // Get the number of points within this VolBox.
        unsigned int VolBox::size() {
            return m_bucket_points.size();
        }
        ...
        inline static void validate_bits(BitVector& bits, unsigned int size)
        {
            if (bits.size() < size) {
                ostringstream outs;
                outs << "set_bits_for_sphere:  not enough bits (" << bits.size() << ").  Need at least " << size << " bits.";
                throw std::invalid_argument(outs.str());
            }
        }
        
        void VolBox::get_points_within_spheres(
            const PointList& spheres, PointList& contained_points,
        ...
        void VolBox::set_bits_for_spheres(const PointList& spheres,
            BitVector& bits, bool from_scratch, unsigned int offset) const
        {
            if (from_scratch) {
        ...
        void VolBox::set_folded_bits_for_spheres(const PointList& spheres,
            BitVector& bits, unsigned int num_folds, unsigned int offset) const
        {
