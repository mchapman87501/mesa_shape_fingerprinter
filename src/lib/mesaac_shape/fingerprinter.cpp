...
namespace mesaac {
    namespace shape {
        ...
        Fingerprinter::Fingerprinter(const VolBox& volbox):
            m_volbox(volbox)
        {
        }
        ...
        static inline void get_point_list(
            const AtomVector& atoms, PointList& result)
        {
            result.clear();
        ...
        void Fingerprinter::compute(
            const AtomVector& atoms, ShapeFingerprint& result)
        {
            result.clear();
        ...
        static inline void get_flipped_points(
            const PointList& points, const float *flip, PointList& flipped)
        {
            flipped = points;
        ...
        void Fingerprinter::compute_for_flip(
            const PointList& points, unsigned int i_flip, Fingerprint& result)
        {
            result.reset();
            PointList flipped;
