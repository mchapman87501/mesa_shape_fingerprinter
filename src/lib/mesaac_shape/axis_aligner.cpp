...
namespace mesaac {
    namespace shape {
        ...
        AxisAligner::AxisAligner(const AxisAligner& src):
        ...
        void AxisAligner::untranslate_points(
            PointList& points, const Point& offset)
        {
            ...
            const PointList& atom_centers)
        {
            if (atoms.size() != atom_centers.size()) {
                ...
                msg << "Atom vector length " << atoms.size() 
                    << " must equal atom centers length " 
                    << atom_centers.size();
        ...
        static inline void transform_point(
            Transform& vt, Point& p)
        {
            Point untransformed(p);
            for (unsigned int j = 0; j != 3; j++) {
                p[j] = ((vt(j, 0) * untransformed[0]) +
                        (vt(j, 1) * untransformed[1]) +
                        (vt(j, 2) * untransformed[2]));
            }
        }
    
        void AxisAligner::transform_points(
            PointList& points, Transform& vt) 
        {
            PointList::iterator i;
            for (i = points.begin(); i != points.end(); ++i) {
        ...
        static inline void get_cross_prod(
            const Point& a, const Point& b, Point& xp)
        {
            xp.clear();
            xp.resize(3);
        ...
        static inline void unmirror_axis(
            Transform& vt, unsigned int i_axis)
        {
            for (unsigned int i = 0; i != 3; i++) {
