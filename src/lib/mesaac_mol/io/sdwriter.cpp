...
#include "mesaac_mol/io/sdwriter.h"
...
namespace mesaac {
    namespace mol {
        ...
        string SDWriter::uint_str(unsigned int u, 
        ...
        bool SDWriter::write(Mol& mol) {
            ...
            m_outf << mol.name() << endl 
                   ...
                   << "  " << program_name << molfile_timestamp() 
                   << setw(1) << d << "D" << endl
                   << mol.comments() << endl 
            ...
            for (i = mol.atoms().begin(); i != mol.atoms().end(); ++i) {
                ...
                if (!a) {
                    ...
                    m_outf << f_str(a->x()) << f_str(a->y()) << f_str(a->z())
                           << " " << setw(3) << left << a->symbol()
                           << a->optional_cols() << endl;
                }
            }
            ...
            for (j = mol.bonds().begin(); j != mol.bonds().end(); ++j) {
                ...
                if (!b) {
                    ...
                    m_outf << uint_str(b->a0()) << uint_str(b->a1()) 
                           << uint_str(b->type()) << uint_str(b->stereo())
                           << b->optional_cols() << endl;
                }
            }
            ...
            for (k = tags.begin(); k != tags.end(); ++k) {
                ...
                while ((value.size() > 0) && (value.find_last_of("\n\t ") == value.size() - 1)) {
