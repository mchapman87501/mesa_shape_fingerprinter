...
#include "mesaac_mol/io/sdreader.h"
...
namespace mesaac {
    namespace mol {
        ...
        bool SDReader::double_field(
            string& line, unsigned int i_start, unsigned int i_len, double& value) {
            m_nums.str(line.substr(i_start, i_len));
            m_nums.clear();
            bool result = (m_nums >> value);
            return result;
        }
    
        bool SDReader::uint_field(
            string& line, unsigned int i_start, unsigned int i_len, 
            unsigned int& value)
        {
            m_nums.str(line.substr(i_start, i_len));
            m_nums.clear();
                                  ...
                                  string& line)
        {
            ...
            if (getline(line)) {
                if (line.size() < 38) {
                    cerr << file_pos()
                         << "WARNING: SD counts line has incorrect length" << endl
        ...
        bool SDReader::read_next_atom(Atom& a) {
            ...
            if (line.size() < 34) {
                cerr << file_pos() << "Atom line is too short: '" << line 
                     << "'." << endl;
            } else {
                double x, y, z;
                if (double_field(line, 0, 10, x) && double_field(line, 10, 10, y) &&
                    ...
                    cerr << file_pos() << "Could not parse coordinates from '"
                         << line << "'." << endl;
                }
            }
            return result;
        ...
        bool SDReader::read_next_bond(Bond& b) {
            ...
            getline(line);
            if (line.size() < 12) {
                cerr << file_pos() << "Warning: Bond line is too short (" 
                     << line.size() << " characters): '" << line 
                     << "'." << endl;
            }
            ...
            // Try to read anyway:
            {
                ...
                if (uint_field(line, 0, 3, a0) && 
                      uint_field(line, 3, 6, a1) &&
                    ...
                    cerr << file_pos() << "Could not parse bond from '"
                         << line << "'." << endl;
                }
            }
            return result;
            ...
            if (m_inf) {
                ...
                while (m_inf && !result) {
                    ...
                    if (getline(mol_name)) {
                        ...
                        result = (read_atoms(next, num_atoms) && 
                                  read_bonds(next, num_bonds)  &&
                                  read_properties_block(next) &&
                                  read_tags(next));
                        if (!result) {
                            cerr << "Could not read molecule starting at line " 
                                 << start_line << "; skipping ahead." << endl;
                            skip_to_end();
                        }
                    }
                }
